...
def module_dashboard(request, module):
    ...
    if module == 'distance_online': counts = {'Distance / Online Education': DistanceOnlineEducation.objects.count(), 'Published': DistanceOnlineEducation.objects.filter(active=True).count(), 'Featured': DistanceOnlineEducation.objects.filter(featured=True).count(), 'Enquiries': Lead.objects.filter(preferred_distance_online__isnull=False).count()}
    return render(request,'admissions/module_dashboard.html',{'module_key':module,'module_title':item[1],'module_description':item[2],'counts':counts,'is_crm_admin':admin_authorised(request.user),'can_manage_distance':request.user.is_staff and request.user.has_perm('admissions.view_distanceonlineeducation'),'can_add_distance':request.user.is_staff and request.user.has_perm('admissions.add_distanceonlineeducation')})
